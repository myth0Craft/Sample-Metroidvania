...
{
    [SerializeField] ShockwaveRenderFeatureSettings settings;

    private ShockwaveRenderFeaturePass mainPass;
    ...
    private void OnBeginCameraRendering(ScriptableRenderContext ctx, Camera cam)
    {
        ...
        hiddenCam.cullingMask = settings.layers;
    ...
    public override void Create()
    {
        ...
        if (needsRealloc)
        {
            ...
            mainPass = new ShockwaveRenderFeaturePass(settings, handle, this);
            mainPass.InvalidateRTInfo();
        }

        if (mainPass != null)
            mainPass.renderPassEvent = settings.renderBehindScene
    ...
    [Serializable]
    public class ShockwaveRenderFeatureSettings
    {
    ...
    class ShockwaveRenderFeaturePass : ScriptableRenderPass
    {
        private readonly RTHandle rt;
        readonly ShockwaveRenderFeatureSettings settings;

        private RenderTargetInfo _rtInfo;
        private ImportResourceParams _importParams;
        private bool _rtInfoDirty = true;
        private ShockwaveRenderFeature feature;

        public ShockwaveRenderFeaturePass(ShockwaveRenderFeatureSettings settings, RTHandle handle, ShockwaveRenderFeature feature)
        {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            if (settings.material.GetFloat("_GlobalAlpha") <= 0) return;
            ...
            using (var builder = renderGraph.AddRasterRenderPass<PassData>("ShockwavePass", out var passData))
            {
